...
if (!isset($_SESSION['csrf_secret'])) {
    $_SESSION['csrf_secret'] = bin2hex(random_bytes(8));
    $csrf_salt = strval(rand(10000,99999));
    $csrf_token = $csrf_salt . ":" . md5($csrf_salt . ":" . $_SESSION['csrf_secret']);
}
// В суперглобальном массиве $_SERVER PHP сохраняет некторые заголовки запроса HTTP
// и другие сведения о клиненте и сервере, например метод текущего запроса $_SERVER['REQUEST_METHOD'].

function filter_input_html($input) {
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (!empty($_COOKIE['save'])) {
        ...
        if (!empty($_COOKIE['login'])) {
            $messages[] = sprintf('Вы можете <a href="login.php" class="login-link">войти</a> с логином <strong>%s</strong>
    ...
    $csrf_salt = substr($_POST['csrf_token'],0,strpos($_POST['csrf_token'], ":"));
    if ($_POST['csrf_token'] !== $csrf_salt . ":" . md5($csrf_salt . ":" . $_SESSION['csrf_secret'])) {
        die('Invalid CSRF token');
    }
    if (parse_url($_SERVER['HTTP_REFERER'], PHP_URL_HOST) !== 'u52816.kubsu-dev.ru') {
        die('Invalid referer');
    }
    ...
    if (empty($_POST['name']) || !preg_match('/^[A-Z][a-z]+$/AD', $_POST['name'])) {
        ...
        setcookie('name_value', filter_input_html($_POST['name']), time() + 30 * 24 * 60 * 60);
    }
    if (empty($_POST['email']) || !preg_match("/^[a-z0-9!#$%&'*+\\/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+\\/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/", $_POST['email'])) {
        setcookie('email_error', '1', time() + 24 * 60 * 60);
        $errors = TRUE;
    } else {
        setcookie('email_value', filter_input_html($_POST['email']), time() + 30 * 24 * 60 * 60);
    }
    if (empty($_POST['year']) || !preg_match("/^(19|20)\d{2}$/", $_POST['year'])) {
        setcookie('year_error', '1', time() + 24 * 60 * 60);
        $errors = TRUE;
    } else {
        setcookie('year_value', filter_input_html($_POST['year']), time() + 30 * 24 * 60 * 60);
    }
    if (!isset($_POST['gender']) || ($_POST['gender']!='0' && $_POST['gender']!='1')) {
        setcookie('gender_error', '1', time() + 24 * 60 * 60);
        $errors = TRUE;
    } else {
        setcookie('gender_value', filter_input_html($_POST['gender']), time() + 30 * 24 * 60 * 60);
    }
    if (!isset($_POST['limbs']) || !preg_match('/^[1234]$/AD', $_POST['limbs'])) {
        setcookie('limbs_error', '1', time() + 24 * 60 * 60);
        $errors = TRUE;
    } else {
        setcookie('limbs_value', filter_input_html($_POST['limbs']), time() + 30 * 24 * 60 * 60);
    }
    ...
    */
    ...
    if (!empty($_COOKIE[session_name()]) &&
        ...
        $stmtErr = $stmt->execute(['p_id' => $_SESSION['uid'], 'name' => filter_input_html($_POST['name']),'mail' => filter_input_html($_POST['email']) , 'year' => filter_input_html($_POST['year']), 'gender' => filter_input_html($_POST['gender']), 'limbs_num' => filter_input_html($_POST['limbs']), 'biography' => filter_input_html($_POST['biography'])]);
        ...
        try {
            ...
            $stmtErr =  $stmt -> execute(['name' => filter_input_html($_POST['name']),'mail' => filter_input_html($_POST['email']) , 'year' => filter_input_html($_POST['year']), 'gender' => filter_input_html($_POST['gender']), 'limbs_num' => filter_input_html($_POST['limbs']), 'biography' => filter_input_html($_POST['biography']),'p_login' => $login, 'p_pass' => $passcode]);
            if (!$stmtErr) {
                header("HTTP/1.1 500 Some server issue");
                exit();
            }
